...
case `arch` in
  i386 )
    install_rpm libdc1394-2.1.2-3.4.el6.i686.rpm
    [ $? -ne 0 ] && exit 1
    install_rpm libmusicbrainz-2.1.5-11.1.el6.i686.rpm
    [ $? -ne 0 ] && exit 1
    sudox yum -y install gstreamer-plugins-bad
    [ $? -ne 0 ] && exit 1
    ;;
  x86_64 )
    install_rpm libdc1394-2.1.2-3.4.el6.x86_64.rpm
    [ $? -ne 0 ] && exit 1
    install_rpm libmusicbrainz-2.1.5-11.1.el6.x86_64.rpm
    [ $? -ne 0 ] && exit 1
    sudox yum -y install gstreamer-plugins-bad
    [ $? -ne 0 ] && exit 1
    ;;
