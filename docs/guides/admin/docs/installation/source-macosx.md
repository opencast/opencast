...
Required (not necessarily on the same machine):
    ...
    OpenSearch 3.x and analysis-icu plugin
...
If you want to install OpenSearch in the same machine run OpenSearch as a Docker container
    ...
    docker run -d --name opensearch -p 9200:9200 -p 9300:9300 -e 'discovery.type=single-node' opensearchproject/opensearch:3
