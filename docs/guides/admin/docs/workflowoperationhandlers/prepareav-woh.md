...
|mux-encoding-profile    |mux-av.prepare-av    |The encoding profile to use for media that needs to be muxed (default is 'mux-av.select-streams')|
|audio-video-encoding-profile    |av.prepare-av    |The encoding profile to use for media that is audio-video already and needs to be re-encodend (default is av.work)     |
|video-encoding-profile    |video-only.prepare-av    |The encoding profile to use for media that is only video and needs to be re-encodend (default is video-only.select-streams)     |
|audio-encoding-profile    |audio-only.prepare-av    |The encoding profile to use for media that is only audio and needs to be re-encodend (default is audio-only.select-streams)     |
