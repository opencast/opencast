...
|apply-acl         |`true`                                      |Whether the ACL should be applied or not. Note: Existing episode ACLs may still influence the effective permissions depending on merge rules in `etc/org.opencastproject.authorization.xacml.XACMLAuthorizationService.cfg`.|`false`|
