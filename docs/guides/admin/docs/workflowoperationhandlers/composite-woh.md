...
one video, including encoding to different formats. The audio track is always taken from the both videos by default. Everything is
...
If both upper and lower tracks have audio, "source-audio-name" can be set to "upper" or "lower" to choose only the
audio from that track for the composite video.
If this parameter is omitted, audio from both tracks are mixed into the composite media.
...
|------------------|-----------------|-------|-----------|-------------|
|source-audio-name|String , { "," , String }    |upper|The "name" of track to use as a source audio.|EMPTY|
