...
Example:
```bash
cd modules/matterhorn-admin-ui
mvn install
cp ./target/matterhorn-admin-ui-2.2-SNAPSHOT.jar ../../build/opencast-dist-allinone-2.2-SNAPSHOT/system/org/opencastproject/matterhorn-admin-ui/2.2-SNAPSHOT/matterhorn-admin-ui-2.2-SNAPSHOT.jar
```
...
testing JavaScript code), and live in `modules/matterhorn-admin-ui/src/test/resources/test/unit`.
