...
var ocScheduler = (function() {
  ...
  sched.registerEventHandlers = function registrerEventHandlers(){
    ...
    $('#seriesSelect').focusout(function() {
      if($('#seriesSelect').val() !== ''){
        ocUtils.log("Searching for series in series endpoint");
        $.ajax({
          url : SERIES_SEARCH_URL + '?seriesTitle=' + $('#seriesSelect').val() + '&edit=true',
          type : 'get',
          dataType : 'json',
          success : function(data) {
            var series_list = data["catalogs"];

            if(series_list.length !== 0){
              $.each(series_list, function(key, value) {
                if(value[DUBLIN_CORE_NS_URI]["title"][0].value == $('#seriesSelect').val()) {
                  series_id = value[DUBLIN_CORE_NS_URI]["identifier"] ? series_list[0][DUBLIN_CORE_NS_URI]["identifier"][0].value : "";
    ...
    if(status == "success") {
      ...
      $('#content').load('complete_scheduling.html', function() {
        ...
        for (var i in sched.catalogs) {
          ...
          for (var key in data) {
            if (data[key] != "" && key != 'files' && data[key].value != "") { //print text, not file names
              $('#field-'+key).css('display','block');
              $('#field-'+key).children('.fieldValue').text(data[key].value);
            }  
            if(key == "temporal") {
              fields = data[key]['fields'];
              duration = fields['durationHour'].val() * 3600;
              duration += fields['durationMin'].val() * 60;
              duration = ocUtils.formatSeconds(duration);
              
              recordDate = new Date(fields['startDate'].val()).getTime() / 1000;  	// Get date in milliseconds, convert to seconds.      
              recordDate += fields['startTimeHour'].val() * 3600; // convert hour to seconds, add to date.      
              recordDate += fields['startTimeMin'].val() * 60; //convert minutes to seconds, add to date.		
              ...
              $('#field-recordStart').css('display','block');
              $('#field-recordStart').children('.fieldValue').text(recordDate);
              
              $('#field-recordTime').css('display','block');
              $('#field-recordTime').children('.fieldValue').text(duration);
            }
          }
        }
      });
