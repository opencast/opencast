...
class WowzaAdaptiveStreamingDistributionRestService : AbstractJobProducerEndpoint() {
    ...
    fun distribute(@FormParam("mediapackage") mediaPackageXml: String,
                   @FormParam("channelId") channelId: String,
    ...
    @Path("/distributesync")
    @Produces(MediaType.TEXT_XML)
    @RestQuery(name = "distributesync", description = "Syncrhonously distribute a media package element to this distribution channel", returnDescription = "The distribution", restParameters = [RestParameter(name = "mediapackage", isRequired = true, description = "The mediapackage", type = Type.TEXT), RestParameter(name = "channelId", isRequired = true, description = "The publication channel ID", type = Type.TEXT), RestParameter(name = "elementIds", isRequired = true, description = "The elements to distribute as Json Array['IdOne','IdTwo']", type = Type.STRING)], reponses = [RestResponse(responseCode = SC_OK, description = "An XML representation of the distribution"), RestResponse(responseCode = SC_NO_CONTENT, description = "There is no streaming distribution service available")])
    @Throws(Exception::class)
    fun distributeSync(@FormParam("mediapackage") mediaPackageXml: String,
        ...
        try {
            ...
            return if (result == null || result.isEmpty()) {
                ...
                Response.ok(MediaPackageElementParser.getArrayAsXml(result)).build()
            }
        } catch (e: IllegalArgumentException) {
            logger.debug("Unable to distribute element: {}", e.message)
            return status(Status.BAD_REQUEST).build()
    ...
    @Throws(Exception::class)
    fun retract(@FormParam("mediapackage") mediaPackageXml: String,
                @FormParam("channelId") channelId: String,
                @FormParam("elementIds") elementIds: String): Response {
        var job: Job? = null
        try {
            ...
            logger.warn("Unable to retract mediapackage '{}' from streaming channel: {}", *arrayOf<Any>(mediaPackageXml, e))
            return Response.serverError().status(Status.INTERNAL_SERVER_ERROR).build()
        }
    ...
    @Throws(Exception::class)
    fun retractSync(@FormParam("mediapackage") mediaPackageXml: String,
                    @FormParam("channelId") channelId: String,
                    @FormParam("elementIds") elementIds: String): Response {
        var result: List<MediaPackageElement>? = null
        try {
    ...
    companion object {
        ...
        /** The logger  */
        private val logger = LoggerFactory.getLogger(WowzaAdaptiveStreamingDistributionRestService::class.java)
