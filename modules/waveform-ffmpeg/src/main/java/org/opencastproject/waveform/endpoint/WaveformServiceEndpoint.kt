...
class WaveformServiceEndpoint : AbstractJobProducerEndpoint() {
    ...
    override val service: JobProducer?
        get() = if (waveformService is JobProducer) {
            waveformService as JobProducer?
        } else {
            null
        }
    ...
    fun createWaveformImage(@FormParam("track") track: String,
        ...
        try {
            ...
            val job = waveformService!!.createWaveformImage(sourceTrack as Track, pixelsPerMinute, minWidth, maxWidth, height, color)
            return Response.ok().entity(JaxbJob(job)).build()
        } catch (ex: WaveformServiceException) {
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(WaveformServiceEndpoint::class.java)
    }
}
