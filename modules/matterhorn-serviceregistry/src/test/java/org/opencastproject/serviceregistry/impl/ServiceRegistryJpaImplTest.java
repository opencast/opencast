...
public class ServiceRegistryJpaImplTest {
  ...
  public void setUpQuery() {
    ...
    EasyMock.expect(query.getSingleResult()).andReturn(EasyMock.createMock(HostRegistration.class));
  ...
  public void setUpPersistenceProvider() {
    ...
    EasyMock.expect(persistenceProvider.createEntityManagerFactory("org.opencastproject.serviceregistry", persistenceProperties)).andReturn(emf);
  ...
  private void setupBundleContext() throws InvalidSyntaxException {
    ...
    EasyMock.expect(bundleContext.createFilter((String)EasyMock.anyObject())).andReturn(EasyMock.createNiceMock(Filter.class));
  ...
  @SuppressWarnings("unchecked")
  @Test
  public void nullContextActivatesOkay() {
    serviceRegistryJpaImpl.scheduledExecutor = EasyMock.createMock(ScheduledExecutorService.class);
    
    EasyMock.expect(serviceRegistryJpaImpl.scheduledExecutor.scheduleWithFixedDelay((JobDispatcher) EasyMock.anyObject(),
            EasyMock.eq(ServiceRegistryJpaImpl.DEFAULT_DISPATCH_PERIOD), EasyMock.eq(ServiceRegistryJpaImpl.DEFAULT_DISPATCH_PERIOD), EasyMock.eq(TimeUnit.MILLISECONDS))).andReturn(
    EasyMock.createNiceMock(ScheduledFuture.class));
    
    EasyMock.expect(serviceRegistryJpaImpl.scheduledExecutor.scheduleWithFixedDelay((JobProducerHearbeat) EasyMock.anyObject(),
            EasyMock.eq(ServiceRegistryJpaImpl.DEFAULT_HEART_BEAT), EasyMock.eq(ServiceRegistryJpaImpl.DEFAULT_HEART_BEAT), EasyMock.eq(TimeUnit.MINUTES))).andReturn(
    EasyMock.createNiceMock(ScheduledFuture.class));
