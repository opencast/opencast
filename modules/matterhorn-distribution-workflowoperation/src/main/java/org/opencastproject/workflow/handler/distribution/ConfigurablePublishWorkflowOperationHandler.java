...
/**
 * WOH that distributes selected elements to an internal distribution channel
 * and adds reflective publication elements to the media package.
 */
public class ConfigurablePublishWorkflowOperationHandler extends AbstractWorkflowOperationHandler {
    ...
    /**
     * The template key for adding the mediapackage / event id to the
     * publication path.
     */
    protected static final String EVENT_ID_TEMPLATE_KEY = "event_id";
    /**
     * The template key for adding the player location path to the publication
     * path.
     */
    ...
    /**
     * Replace possible variables in the url-pattern configuration for this
     * workflow operation handler.
     *
     * @param urlPattern The operation's template for replacing the variables.
     * @param mp The {@link MediaPackage} used to get the event / mediapackage
     * id.
     * @param pubUUID The UUID for the published element.
     * @return The URI of the published element with the variables replaced.
     * @throws WorkflowOperationException Thrown if the URI is malformed after
