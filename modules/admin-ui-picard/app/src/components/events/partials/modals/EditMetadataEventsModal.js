            ...
            <section className="modal wizard modal-animation">
                        ...
                        <div className="modal-body">
                            <div className="row">
                                <div className="alert sticky error">
                                    <p>{t('BULK_ACTIONS.EDIT_EVENTS_METADATA.FATAL_ERROR', {fatalError: fatalError.fatalError})}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {!loading && fatalError.fatalError === undefined && (
                    <MuiPickersUtilsProvider utils={DateFnsUtils} locale={currentLanguage.dateLocale}>
                        <Formik initialValues={fetchedValues}
                                                ...
                                                <div className="obj tbl-details">
                                                    ...
                                                    <div className="obj-container">
                                                        <table className="main-tbl">
                                                            ...
                                                            <tbody>
                                                            {metadataFields.mergedMetadata.map((metadata, key) => (
                                                                !metadata.readOnly && (
                                                                    <tr key={key} className={cn({info: metadata.differentValues})}>
                                                                        <td>
                                                                            <input type="checkbox"
                                                                                   name="changes"
                                                                                   checked={isTouchedOrSelected(metadata, formik.values)}
                                                                                   disabled={((!metadata.differentValues && !metadata.selected)
                                                                                       || (metadata.required && !metadata.selected))}
                                                                                   onChange={e => onChangeSelected(e, metadata.id)}
...
const mapDispatchToProps = dispatch => ({
    updateBulkMetadata: (metadataFields, values) => dispatch(updateBulkMetadata(metadataFields, values))
})
export default connect(mapStateToProps, mapDispatchToProps)(EditMetadataEventsModal);
