 ...
 */
const EventDetailsWorkflowTab = ({ eventId, t, close, setHierarchy, baseWorkflow, saveWorkflowConfig, removeNotificationWizardForm,
    ...
    const setInitialValues = () => {
        ...
        return {
            workflowDefinition: !!workflow.workflowId ? workflow.workflowId : baseWorkflow.workflowId,
                                                    ...
                                                    {isRoleWorkflowEdit && (
                                                        <td>
                                                            {(item.status === 'EVENTS.EVENTS.DETAILS.WORKFLOWS.OPERATION_STATUS.RUNNING') && (
                                                                <a onClick={() => workflowAction(item.id, 'STOP')}
                                                            ...
                                                            {(item.status === 'EVENTS.EVENTS.DETAILS.WORKFLOWS.OPERATION_STATUS.PAUSED') && (
                                                                <a onClick={() => workflowAction(item.id, 'NONE')}
                                                            ...
                                                            {(item.status === 'EVENTS.EVENTS.DETAILS.WORKFLOWS.OPERATION_STATUS.PAUSED') && (
                                                                <a onClick={() => workflowAction(item.id, 'RETRY')}
                                                            ...
                                                            {((item.status === 'EVENTS.EVENTS.DETAILS.WORKFLOWS.OPERATION_STATUS.SUCCEEDED'
                                                                ...
                                                                && !isCurrentWorkflow(item.id) && isRoleWorkflowDelete)
                                                                && (
                                                                    <a onClick={() => deleteWorkflow(item.id)}
                                                                    ...
                                                                    <td>
                                                                        <div className="obj-container padded">
                                                                            <div className="editable">
                                                                                <DropDown value={formik.values.workflowDefinition}
                                                                                          text={workflowDefinitions.find(workflowDef => workflowDef.id === formik.values.workflowDefinition).title}
                                                                                          options={ (!!workflowDefinitions && workflowDefinitions.length > 0) ?
                                                                                          ...
                                                                                          handleChange={element =>
                                                                                              changeWorkflow(element.value, formik.setFieldValue)
                                                                                          }
                                                                                          placeholder={ (!!workflowDefinitions && workflowDefinitions.length > 0) ?
                                                                    ...
                                                                    <td>
                                                                        <div className="obj-container padded">
                                                                            {(hasCurrentAgentAccess() && isRoleWorkflowEdit && !!workflowConfiguration && !!workflowConfiguration.workflowId) && (
                                                                                ...
                                                                                >
                                                                                    <RenderWorkflowConfig workflowId={workflowConfiguration.workflowId}
                                                                                                          formik={formik} />
                                                                                </div>
                                                                            )}
                                                ...
                                                {(hasCurrentAgentAccess() && isRoleWorkflowEdit && !!workflowConfiguration && !!workflowConfiguration.workflowId && formik.dirty) && (
                                                    <footer style={{ padding: '15px' }}>
                                                        <div className="pull-left">
                                                            <button type="reset"
                                                                    onClick={() => {
                                                                        changeWorkflow(baseWorkflow.workflowId, formik.setFieldValue);
                                                                        formik.resetForm();
                                                                    }}
                                                                    disabled={!formik.isValid}
                                                                    className={`cancel  ${!formik.isValid ? 'disabled' : ''}`}
                                                            >
                                                                {t('CANCEL')/* Cancel */}
                                                            </button>
                                                        </div>
                                                        <div className="pull-right">
                                                            <button onClick={() => formik.handleSubmit()}
                                                                    ...
                                                                    className={`save green  ${!(formik.dirty && formik.isValid) ? 'disabled' : ''}`}
                                                            >
                                                                {t('SAVE')/* Save */}
                                                            </button>
                                                        </div>
...
const mapDispatchToProps = dispatch => ({
    ...
    saveWorkflowConfig: (values, eventId) => dispatch(saveWorkflowConfig(values, eventId)),
