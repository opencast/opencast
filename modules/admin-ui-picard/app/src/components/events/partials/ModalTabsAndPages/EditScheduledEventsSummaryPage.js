        ...
        for (let i = 0; i < formik.values.editedEvents.length; i++) {
            ...
            if(isChanged(event.startTimeHour + ':' + event.startTimeMinutes,
                ...
                eventChanges.changes.push({
                    ...
                    next: event.changedStartTimeHour + ':' + event.changedStartTimeMinutes
                });
            }
            if(isChanged(event.endTimeHour + ':' + event.endTimeMinutes,
            ...
            if(eventChanges.changes.length > 0) {
                ...
                formik.setFieldValue('changedEvent', formik.values.changedEvents.push(event.eventId));
                        ...
                        <div className="full-col">
                            ...
                            {changes.map((event, key) => (
                                <div key={key} className="obj tbl-list">
                                    <header>{t('BULK_ACTIONS.EDIT_EVENTS.SUMMARY.SINGLE_EVENT_CAPTION', {title: event.title})}</header>
