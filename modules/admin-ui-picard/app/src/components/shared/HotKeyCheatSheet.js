                        ...
                        <div className="full-col">
                            ...
                            {Object.keys(availableHotkeys).map((hotkeyGroup, key) => (
                                <div className="obj tbl-list" key={key}>
                                    <header>{t('HOTKEYS.GROUPS.' + hotkeyGroup.toUpperCase())}</header>
                                    <table className="main-tbl">
                                        <tbody>
                                        {/* Repeat row for each hotkey in group*/}
                                        {Object.keys(availableHotkeys[hotkeyGroup]).map((hotkey, key) => (
                                            <tr key={key}>
                                                <td className="hotkey">
                                                    <p className="combo">
                                                        <span className="chord">
                                                            {/* repeat for each key in hotkey */}
                                                            {availableHotkeys[hotkeyGroup][hotkey].combo.map((comboKey, key) => (
                                                                <>
                                                                    <span key={key}>
                                                                        <span className="key">{t('HOTKEYS.KEYS.' + comboKey.toUpperCase(), comboKey)}</span>
                                                                    </span>
                                                                    {comboKey === availableHotkeys[hotkeyGroup][hotkey].combo[availableHotkeys[hotkeyGroup][hotkey].combo.length - 1]
                                                                        ? ''
                                                                        : ' + '}
                                                                </>
                                                            ))}
