...
const TableFiltersProfiles = ({showFilterSettings, setFilterSettings, createFilterProfile, filterMap,
    ...
    const handleChange = e => {
        ...
        if (itemName === "name") {
            const isDuplicated =
                profiles.some(profile => profile.name === itemValue);
            if (!isDuplicated) {
                setValidName(true);
            } else {
                setValidName(false);
        ...
        <div className="btn-dd filter-settings-dd df-profile-filters">
            ...
            {settingsMode ? (
                        ...
                        {currentProfiles.length === 0 ? (
                            ...
                            currentProfiles.map((profile, key) => (
                                <li key={key}>
                                    ...
                                    <a onClick={() => editFilterProfile(profile)}
                                       title={t('TABLE_FILTERS.PROFILES.EDIT')}
                                       className="icon edit"/>
                                    {/* Remove icon to remove profile */}
                                    <a onClick={() => removeFilterProfile(profile)}
                ...
                <div className="filter-details">
                    <header>
                        <a className="icon close" onClick={() => {
                            setFilterSettings(!showFilterSettings)
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            name="description"
                            onChange={e => handleChange(e)}
                            placeholder={t('TABLE_FILTERS.PROFILES.DESCRIPTION_PLACEHOLDER')} />
                    </div>
                    <div className="input-container">
                        {/* Buttons for saving and canceling editing */}
                        <div className="btn-container">
                            <a onClick={cancelEditProfile} className="cancel">{t('CANCEL')}</a>
                            <a onClick={saveProfile} className={cn("save", {disabled: !validName})} >{t('SAVE')}</a>
                        </div>
                    </div>
                </div>
