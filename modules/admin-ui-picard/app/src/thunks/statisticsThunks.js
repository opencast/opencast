        ...
        .then( response => {
            ...
            for(let i = 0; i < response.data.length; i++){
                ...
                if(response.data[i].providerType !== 'timeSeries'){
                    ...
                    const csvUrl = createDownloadUrl(resourceId, resourceType, response.data[i].providerId,
                        from, to, dataResolution);
...
export const fetchStatisticsValueUpdate = (resourceId, resourceType, providerId, from, to, dataResolution, timeMode,
    ...
    axios.post('/admin-ng/statistics/data.json', requestData, requestHeaders)
        .then(dataResponse => {
            ...
            if(dataResponse.data.length === 1){
                ...
                const options = createChartOptions(timeMode, dataResolution);
                const csvUrl = createDownloadUrl(resourceId, resourceType, providerId, from, to, dataResolution);
