...
.factory('EventsResource', ['$resource', 'Language', '$translate', 'ResourceHelper', function ($resource, Language,
  $translate, ResourceHelper) {
  ...
  return $resource('/admin-ng/event/:id', { id: '@id' }, { query: {method: 'GET', params: { id: 'events.json' },
    isArray: false, cancellable: true, transformResponse: function (data) {
        ...
        if (typeof(r.publications) != 'undefined' && r.publications != null) {
          ...
          for (var i = 0; i < row.publications.length; i++)
            if (row.publications[i].id == 'engage-live' &&
              ...
              row.publications[i].enabled = true;
            }
        }
