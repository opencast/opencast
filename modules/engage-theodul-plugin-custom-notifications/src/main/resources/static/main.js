...
define(['require', 'jquery', 'backbone', 'engage/core'], function(require, $, Backbone, Engage) {
  ...
  var events = {
    ...
    bufferedAndAutoplaying: new Engage.Event('Video:bufferedAndAutoplaying',
      'buffering successful, was playing, autoplaying now', 'handler'),
    bufferedButNotAutoplaying: new Engage.Event('Video:bufferedButNotAutoplaying',
      'buffering successful, was not playing, not playing now', 'handler'),
    ...
    audioCodecNotSupported: new Engage.Event('Video:audioCodecNotSupported',
      'when the audio codec seems not to be supported by the browser', 'handler')
  };
  ...
  function initPlugin() {
    ...
    window.setTimeout(function() {
      if (!videoLoaded && !mediapackageError && !codecError) {
        ...
        if (!isAudioOnly) {
          let msg = translate('error_videoLoading', 'The video is loading. Please wait a moment.');
          alertify.error(getAlertifyMessage(msg));
        } else {
          let msg = translate('error_audioLoading', 'The audio is loading. Please wait a moment.');
          alertify.error(getAlertifyMessage(msg));
        }
      }
    ...
    Engage.on(plugin.events.ready.getName(), function(query) {
      ...
      if (!videoLoaded && videoLoadMsgDisplayed && !mediapackageError && !codecError) {
        if (!isAudioOnly) {
          let msg = translate('msg_videoLoadedSuccessfully', 'The video has been loaded successfully.');
          alertify.success(getAlertifyMessage(msg));
        } else {
          let msg = translate('msg_audioLoadedSuccessfully', 'The audio has been loaded successfully.');
          alertify.success(getAlertifyMessage(msg));
        }
      }
      videoLoaded = true;
    ...
    Engage.on(plugin.events.buffering.getName(), function() {
      if (!videoBuffering && !mediapackageError && !codecError) {
        videoBuffering = true;
        let msg = translate('msg_videoBuffering', 'The video is currently buffering. Please wait a moment.');
        alertify.success(getAlertifyMessage(msg));
      }
    });
    Engage.on(plugin.events.bufferedAndAutoplaying.getName(), function() {
      if (videoBuffering && !mediapackageError && !codecError) {
        ...
        let msg = translate('msg_videoBufferedSuccessfullyAndAutoplaying',
          'The video has been buffered successfully and is now autoplaying.');
        alertify.success(getAlertifyMessage(msg));
      }
    });
    Engage.on(plugin.events.bufferedButNotAutoplaying.getName(), function() {
      if (videoBuffering && !mediapackageError && !codecError) {
        videoBuffering = false;
        let msg = translate('msg_videoBufferedSuccessfully', 'The video has been buffered successfully.');
        alertify.success(getAlertifyMessage(msg));
      }
    });
    ...
    Engage.on(plugin.events.audioCodecNotSupported.getName(), function() {
      codecError = true;
      let msg = translate('error_AudioCodecNotSupported', 'Error: The audio codec is not supported by this browser');
      alertify.error(getAlertifyMessage(msg));
    });
  }
