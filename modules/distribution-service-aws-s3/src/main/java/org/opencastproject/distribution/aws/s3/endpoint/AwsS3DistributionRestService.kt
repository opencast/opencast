 ...
 */
@Path("/")
@RestService(name = "awss3distributionservice", title = "AWS S3 Distribution Service", abstractText = "This service distributes media package elements to AWS S3.", notes = ["All paths above are relative to the REST endpoint base (something like http://your.server/files)", "If the service is down or not working it will return a status 503, this means the the underlying service is " + "not working and is either restarting or has failed", "A status code 500 means a general failure has occurred which is not recoverable and was not anticipated. In "
        + "other words, there is a bug! You should file an error report with your server logs from the time when the "
        + "error occurred: <a href=\"https://opencast.jira.com\">Opencast Issue Tracker</a>"])
class AwsS3DistributionRestService : AbstractJobProducerEndpoint() {

    /** The distribution service  */
    protected var service: AwsS3DistributionService
     ...
     * the service to set
     */
    fun setService(service: AwsS3DistributionService) {
    ...
    fun distribute(@FormParam("mediapackage") mediaPackageXml: String,
                   @FormParam("channelId") channelId: String, @FormParam("elementId") elementId: String,
                   @DefaultValue("true") @FormParam("checkAvailability") checkAvailability: Boolean): Response {
        var job: Job? = null
        try {
            val gson = Gson()
            val setElementIds = gson.fromJson<Set<String>>(elementId, object : TypeToken<Set<String>>() {
    ...
    @Path("/distributesync")
    @Produces(MediaType.TEXT_XML)
    @RestQuery(name = "distributesync", description = "Synchronously distribute a media package element to this distribution channel", returnDescription = "The distribution", restParameters = [RestParameter(name = "mediapackage", isRequired = true, description = "The mediapackage", type = Type.TEXT), RestParameter(name = "channelId", isRequired = true, description = "The publication channel ID", type = Type.TEXT), RestParameter(name = "elementId", isRequired = true, description = "The element to distribute", type = Type.STRING), RestParameter(name = "checkAvailability", isRequired = false, description = "If the service should try to access the distributed element", type = Type.BOOLEAN)], reponses = [RestResponse(responseCode = SC_OK, description = "An XML representation of the distribution")])
    @Throws(Exception::class)
    ...
    fun retract(@FormParam("mediapackage") mediaPackageXml: String, @FormParam("channelId") channelId: String,
                @FormParam("elementId") elementId: String): Response {
        var job: Job? = null
        try {
            val gson = Gson()
            val setElementIds = gson.fromJson<Set<String>>(elementId, object : TypeToken<Set<String>>() {
    ...
    @Path("/retractsync")
    @Produces(MediaType.TEXT_XML)
    @RestQuery(name = "retractsync", description = "Synchronously retract a media package element from this distribution channel", returnDescription = "The retraction", restParameters = [RestParameter(name = "mediapackage", isRequired = true, description = "The mediapackage", type = Type.TEXT), RestParameter(name = "channelId", isRequired = true, description = "The publication channel ID", type = Type.TEXT), RestParameter(name = "elementId", isRequired = true, description = "The element to retract", type = Type.STRING)], reponses = [RestResponse(responseCode = SC_OK, description = "An XML representation of the retraction")])
    @Throws(Exception::class)
  ...
  */
    ...
    companion object {
        ...
        /** The logger  */
        private val logger = LoggerFactory.getLogger(AwsS3DistributionRestService::class.java)
    }
