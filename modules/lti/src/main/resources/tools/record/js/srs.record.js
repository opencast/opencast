...
var OCRecorder = (function($) {
  ...
  Recorder.prototype = {
    ...
    init: function() {
      ...
      self.$toggleEffectsTool = $('#magicVideoToggle');
      ...
      self.$positions.on('click', '.position', function(event){
        ...
        self.$positions.find('a.position.active').removeClass('active');
      ...
      self.$installer.on('submit', function(event) {
        ...
        if (self.browser == 'chrome' && chrome.webstore) {
          chrome.webstore.install(
            'https://chrome.google.com/webstore/detail/oghkihildgnkedhlolgjomnbfmjpmddm',
            function() {
              location.reload();
            },
            function(e) {
      ...
      self.$toggleEffectsTool.on('change', function(e) {
        if (e.target.checked) {
          self.toggleVideoMagic();
        }
        else {
          self.removeVideoCrop();
        }
      ...
      mediaMgr.activateStream(a.data('id'))
        .then(function(stream) {
          try {
            ...
            compositor.onaudiotrack = function() {
              audioAnalyser.analyse(compositor.stream);
            }
          } catch(streamErr) {
            throw streamErr;
    ...
    toggleVideoCrop: function() {
      this.populateWorkspaceList($('#cropList'));
    },
    toggleVideoMagic: function() {
      this.populateWorkspaceList($('#magicList'));
    },
    populateWorkspaceList: function($el) {
      if ($el.html()) {
        $el.empty();
      }
      ...
      compositor.streams.forEach(function(stream) {
        ...
        $el.append($vidContainer);

        $vidContainer.on({
          click: this.activateCrop.bind(this)
        })
      }.bind(this));
    },
...
/*      this.available = [     
        {id: '2', flavor: 'webcam', source: 'video'},
        {id: '3', flavor: 'microphone', source: 'audio'},
        {id: '4', flavor: 'microphone', source: 'audio'}
      ];*/

      //Note: for FF, user needs to "remember" setting to share devices for actual device name to popup as the label
      devices
...
var runRecorder = new OCRecorder({id: 'recorder'});
