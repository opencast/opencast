ocRecordings = new (function() {
  ...
  var uploadSetting = $.getURLParameter("upload");
  ...
  function refresh() {
    if (!refreshing) {
      ...
      // seriesId should be used instead but the JSON endpoint does not filter by seriesId
 //     var url = WORKFLOW_LIST_URL + '?q=' + seriesID + '&' + params.join('&');
      var url = '/admin-ng/event/events.json?filter=series:' + seriesID;
      $.ajax(
      {
        url: url,
        dataType: 'json',
 ...
 //   this.series = wf.mediapackage.series;
    if (!wf.series || !wf.series.id) return;
    this.series = wf.series.id;
 //   this.seriesTitle = wf.mediapackage.seriestitle;
    this.seriesTitle = wf.series.title;
 ...
 /*   // Creator(s)
    ...
    if (wf.mediapackage.start) {
      this.start = ocUtils.fromUTCDateStringToFormattedTime(wf.mediapackage.start);
    }
*/
    if (wf.presenters.length > 0) {
      this.creators = wf.presenters.join(', ');
    }

    // Start Time
    if (wf.start_date) {
      this.start = ocUtils.fromUTCDateStringToFormattedTime(wf.start_date);
    }
    // Status
    var op = false;
...
/*    if (wf.workflow_state == 'SUCCEEDED') {
      this.state = 'Published';
    } else if (wf.workflow_state == 'FAILING' || wf.workflow_state == 'FAILED') {
      this.state = 'Failed';
      if (wf.errors === '') {
        if (op) {
      ...
      if (op) {
        if (op.id == 'schedule') {
          ...
          this.state = 'On Hold';
          this.operation = "";
          //this.holdAction = { url : op.holdurl, title : op['hold-action-title'] };
        }
      } else {
        ocUtils.log('No current operation for workflow ' + wf.id);
        this.state = 'Paused';
      }
    } else if (wf.workflow_state == 'RUNNING') {
      if (op) {
    ...
    } else if (wf.workflow_state == 'INSTANTIATED') {
      this.state = 'Initializing...';
    } else {
      this.state = wf.workflow_state;
    }
*/
    if (wf.event_status == 'EVENTS.EVENTS.STATUS.PROCESSED') {
      if (wf.scheduling_status == '' || wf.publications.length > 0) {
        this.state = 'Published';
      }
      else if (wf.scheduling_status == 'EVENTS.EVENTS.SCHEDULING_STATUS.READY_FOR_RECORDING') {
        this.state = 'Awaiting Review';
      }
    } else if (wf.event_status == 'EVENTS.EVENTS.STATUS.SCHEDULED') {
      this.state = 'Upcoming';
    } else if (wf.event_status == 'EVENTS.EVENTS.STATUS.INGESTING') {
      this.state = 'Captured';
    } else if (wf.event_status == 'EVENTS.EVENTS.STATUS.PROCESSING_FAILURE') {
      this.state = 'Failed';
      if (wf.errors) {
        if (op) {
          this.error = 'Failed in operation ' + op.description || '';
        ...
        this.error = 'Something';//ocUtils.ensureArray(wf.errors.error).join(', ');
        //TODO: track down jqotesubtpl in source, and get error events to display
      }
      ...
      if (op) {
        if (op.id == 'schedule') {
          ...
          this.state = 'Captured';
          this.operation = 'Sending recording to processing';
        } else if (op.holdurl) {           //not in 2.3.x
          this.state = 'On Hold';
          this.operation = "";
          //this.holdAction = { url : op.holdurl, title : op['hold-action-title'] };
        }
    ...
    } else if (wf.event_status == 'RUNNING') {
      if (op) {
        this.state = 'Processing';
        this.operation = "";
      } else {
...
//    var wfs = ocUtils.ensureArray(data.workflows.workflow);
    var wfs = ocUtils.ensureArray(data.results);
 ...
 //   ocRecordings.totalRecordings = parseInt(data.workflows.totalCount);
    ocRecordings.totalRecordings = parseInt(data.total);
    if(ocRecordings.totalRecordings >= data.count) {
      ocRecordings.currentShownRecordings = parseInt(data.count);
    } else {
      ocRecordings.currentShownRecordings = ocRecordings.totalRecordings;
    }
    var renderedData = makeRenderData(data);
    console.log(renderedData);
    console.log($.fn.jqotesubtpl);
    $('#tableContainer').jqotesubtpl("templates/recordings-table.tpl", renderedData);
...
//    $('#tableContainer').jqotesubtpl("templates/recordings-table.tpl", makeRenderData(data));
    ...
    if (ocRecordings.Configuration.filterText) {
      if (data.workflows.totalCount == '0') {
        $('#filterRecordingCount').css('color', '#CCC');
      } else {
 ...
 //   var pageCount = Math.ceil(data.workflows.totalCount / ocRecordings.Configuration.pageSize);
    var pageCount = Math.ceil(data.total / ocRecordings.Configuration.pageSize);
  ...
  this.reload = function() {
    if($('#holdActionUI').attr('src') == undefined) {
      document.location.href = "index.html?sid="+ seriesID + "&upload=" + uploadSetting + "&" + ocRecordings.buildURLparams();
    } else {
  ...
  this.removeRecording = function(id) {
    if(confirm('Are you sure you wish to delete the recording?')){
      $.ajax({
        url: '/recordings/' + id,
        type: 'DELETE',
        dataType: 'text',
