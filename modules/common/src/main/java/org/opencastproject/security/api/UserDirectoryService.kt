...
/**
 * A marker interface for federation of all [UserProvider]s.
 */
interface UserDirectoryService {
     ...
     * if no organization is set for the current thread
     */
    open fun loadUsers(userNames: Collection<String>): Iterator<User> {
        val result = ArrayList<User>(userNames.size)
