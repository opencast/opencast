...
/**
 * Marshals and unmarshalls [Organization]s to/from XML.
 */
object OrganizationParser {
    ...
    init {
        try {
            jaxbContext = JAXBContext.newInstance("org.opencastproject.security.api",
                    OrganizationParser::class.java!!.getClassLoader())
    ...
    fun fromXml(xml: String): Organization {
        var `in`: InputStream? = null
        try {
            `in` = IOUtils.toInputStream(xml)
            val unmarshaller = jaxbContext.createUnmarshaller()
            return unmarshaller.unmarshal<JaxbOrganization>(StreamSource(`in`), JaxbOrganization::class.java).value
        } catch (e: JAXBException) {
    ...
    fun toXml(organization: Organization): String {
        try {
