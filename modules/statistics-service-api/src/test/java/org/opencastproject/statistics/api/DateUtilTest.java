...
public class DateUtilTest {
  ...
  public void testGetBucketsDaily() {
    for (int i = 0; i < repetitions; i++) {
      ...
      final List<Instant> buckets = DateUtil.getBuckets(from, to, DataResolution.DAILY, ZoneId.of("Z"));
  ...
  public void testGetBucketsWeekly() {
    for (int i = 0; i < repetitions; i++) {
      ...
      final List<Instant> buckets = DateUtil.getBuckets(from, to, DataResolution.WEEKLY, ZoneId.of("Z"));
  ...
  public void testGetBucketsMonthly() {
    for (int i = 0; i < repetitions; i++) {
      ...
      final List<Instant> buckets = DateUtil.getBuckets(from, to, DataResolution.MONTHLY, ZoneId.of("Z"));
      assertEquals(from, buckets.get(0));
      if (buckets.size() == 1) {
        continue;
      }
  ...
  public void testGetBucketsYearly() {
    for (int i = 0; i < repetitions; i++) {
      ...
      final List<Instant> buckets = DateUtil.getBuckets(from, to, DataResolution.YEARLY, ZoneId.of("Z"));
      assertEquals(from, buckets.get(0));
      if (buckets.size() == 1) {
        continue;
      }
