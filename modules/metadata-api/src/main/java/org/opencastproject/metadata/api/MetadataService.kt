...
interface MetadataService<A> {
     ...
     *
     * When more than one MetadataService is registered, the [.getMetadata] method may be
    ...
    /**
     * Gets the metadata for a [MediaPackage] if possible.  If no metadata can be extracted
     * from the catalogs in the [MediaPackage], this returns null;
     *
     * @param mediaPackage The mediapackage to inspect for catalogs
     * @return The metadata extracted from the media package
     */
    fun getMetadata(mediaPackage: MediaPackage): A

    companion object {

        /** The static constant used when configuring the priority  */
        val PRIORITY_KEY = "priority"
    }
}
