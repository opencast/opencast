CREATE TABLE `mh_assets_snapshot` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `UNQ_mh_assets_snapshot` (`mediapackage_id`,`version`),
  KEY `IX_mh_assets_snapshot_archival_date` (`archival_date`),
  KEY `IX_mh_assets_snapshot_mediapackage_id` (`mediapackage_id`),
  KEY `IX_mh_assets_snapshot_organization_id` (`organization_id`),
...
CREATE TABLE `mh_assets_asset` (
  ...
  PRIMARY KEY (`id`),
  KEY `IX_mh_assets_asset_checksum` (`checksum`),
  KEY `IX_mh_assets_asset_mediapackage_element_id` (`mediapackage_element_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
