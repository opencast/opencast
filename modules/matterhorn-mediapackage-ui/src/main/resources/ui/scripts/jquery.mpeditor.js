...
(function($){
	...
	$.fn.mediaPackageEditor = function(options, mp) {
		var self = this;
		var properties = defProperties;
		var originalMediaPackage = new MediaPackage(mp || {});
		var inMemoryMediaPackage = originalMediaPackage.clone();
		var finalMediaPackage;
		...
		this.finishEditing = function() {
		    ...
		    if($.isEmptyObject(inMemoryMediaPackage.seriesCatalog.values) && !$.isEmptyObject(originalMediaPackage.seriesCatalog.values)) {
		...
		this.deleteDCSeries = function(seriesCatalog) {
			...
			if(delCatalog(seriesCatalog)) {
				finalMediaPackage.deleteSeries(seriesCatalog);
			} else {
		...
		this.init = function(){
			...
			$.each(properties.catalogs, function(key, value) { 
				...
				$.each(catalogs, function(i, cat) {
					...
					var tab = element.find('#' + flavor + '_' + cat.id + '_tab');
