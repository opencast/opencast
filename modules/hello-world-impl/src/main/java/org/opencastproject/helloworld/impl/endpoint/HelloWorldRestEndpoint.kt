...
class HelloWorldRestEndpoint {
    ...
    /** The service  */
    protected var helloWorldService: HelloWorldService
    ...
    @RestQuery(name = "helloworld", description = "example service call", reponses = [RestResponse(description = "Hello World", responseCode = HttpServletResponse.SC_OK), RestResponse(description = "The underlying service could not output something.", responseCode = HttpServletResponse.SC_INTERNAL_SERVER_ERROR)], returnDescription = "The text that the service returns.")
    ...
    @Path("helloname")
    @Produces(MediaType.TEXT_PLAIN)
    @RestQuery(name = "helloname", description = "example service call with parameter", restParameters = [RestParameter(description = "name to output", isRequired = false, name = "name", type = RestParameter.Type.TEXT)], reponses = [RestResponse(description = "Hello or Hello Name", responseCode = HttpServletResponse.SC_OK), RestResponse(description = "The underlying service could not output something.", responseCode = HttpServletResponse.SC_INTERNAL_SERVER_ERROR)], returnDescription = "The text that the service returns.")
    @Throws(Exception::class)
    fun helloName(@FormParam("name") name: String): Response {
        logger.info("REST call for Hello Name")
        return Response.ok().entity(helloWorldService.helloName(name)).build()
    }

    fun setHelloWorldService(service: HelloWorldService) {
        this.helloWorldService = service
    }

    companion object {
        /** The logger  */
        private val logger = LoggerFactory.getLogger(HelloWorldRestEndpoint::class.java)
    }
}
