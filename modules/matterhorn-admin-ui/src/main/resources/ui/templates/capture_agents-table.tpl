<div id="stage" class="ui-widget ui-helper-clearfix">
  <table id="captureTable" class="ui-widget" cellspacing="0" width="100%">
    <thead>
      <tr>
        <th id="sortAgent" width="22%" class="ui-widget-header sortable">
          <div>Agent Name<div class="sort-icon ui-icon-triangle-2-n-s"></div></div></th>
        <th id="sortCapabilities" width="50%" class="ui-widget-header sortable">
          <div>Capabilities<div class="sort-icon ui-icon-triangle-2-n-s"></div></div></th>
        <th id="sortStatus" width="22%" class="ui-widget-header sortable">
          <div>Status<div class="sort-icon ui-icon-triangle-2-n-s"></div></div></th>
        <th width="6%" class="ui-widget-header"> </th>
    </thead>
    <tbody>
      ...
      <% $.each(data[j].agents, function(key, agent) { %>
        <tr>
          ...
          <td class="ui-state-active" align=center>
            <a class=rm-ca-btn
              onclick="unregisterCaptureAgent('<%! agent.name %>');"
              title="Unregister Capture-Agent">×</a>
          </td>
        </tr>
      <% }); %>
    <% } else { %>
