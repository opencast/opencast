...
define(['require', 'jquery', 'underscore', 'backbone'], function (require, $, _, Backbone) {
  ...
  var EngageModel = Backbone.Model.extend({
    initialize: function() {
      ...
      //check modus, if no modus param given try to discover browser
      if(this.get("urlParameters").mode === "mobile"){
          this.set("modus", "mobile");
      }else if(this.get("urlParameters").mode === "desktop"){
          this.set("modus", "desktop");
      }else if(this.get("urlParameters").mode === "embed"){
          this.set("modus", "embed");
      }else{
          var isMobile;
          //code from http://detectmobilebrowsers.com/, recommended to update regex expression time to time
          (function(a) {
      ...
      console.log("Player Modus: "+this.get("modus"));
    },
    /*
     * Public properties
     */
