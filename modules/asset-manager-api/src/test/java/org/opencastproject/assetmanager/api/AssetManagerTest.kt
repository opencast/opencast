...
class AssetManagerTest {
    ...
    fun testAssetManagerApiCompilation() {
        ...
        val date: Date? = null // <- assume some real date here
        ...
        val v: Version? = null // <- assume some version here
        ...
        q.select(q.snapshot(), p.start().target()).where(p.start().gt(Date())).run()
    ...
    internal class Props(q: AQueryBuilder) : PropertySchema(q, "org.opencastproject.myservice") {

        // define your properties here below

        fun count(): PropertyField<Long> {
            return longProp("count")
        }

        fun approved(): PropertyField<Boolean> {
            return booleanProp("approved")
        }

        fun start(): PropertyField<Date> {
