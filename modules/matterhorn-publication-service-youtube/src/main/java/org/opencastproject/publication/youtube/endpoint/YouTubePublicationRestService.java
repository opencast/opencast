 ...
 */
@Path("/")
@RestService(name = "youtubepublicationservice", title = "YouTube Publication Service", abstractText = "", notes = {
...
public class YouTubePublicationRestService extends AbstractJobProducerEndpoint {
  ...
  /** The logger */
  private static final Logger logger = LoggerFactory.getLogger(YouTubePublicationRestService.class);
   ...
   *          the service to set
   */
  protected void setService(YouTubePublicationService service) {
  ...
  public Response publish(@FormParam("mediapackage") String mediaPackageXml, @FormParam("elementId") String elementId)
          throws Exception {
    Job job = null;
    try {
      MediaPackage mediapackage = MediaPackageParser.getFromXml(mediaPackageXml);
      Track track = mediapackage.getTrack(elementId);
      if (track != null) {
  ...
  public Response retract(@FormParam("mediapackage") String mediaPackageXml) throws Exception {
    Job job = null;
    try {
      MediaPackage mediapackage = MediaPackageParser.getFromXml(mediaPackageXml);
