...
public abstract class ElementTimeImpl implements ElementTime {
  ...
  /** The logger */
  private static final Logger logger = LoggerFactory.getLogger(ElementTimeImpl.class);
  ...
  public TimeList getBegin() {
    ...
    if (beginTimeList.size() == 0) {
       ...
       * 
       * 1 - ElementSequentialTimeContainer (The SMILDocument) 2 - ElementParallelTimeContainer (A Time-Child of the
       * SMILDocument, which is a seq) 3 - ElementTime (A SMILMediaElement).
       * 
       * 1 - In the first case, the default start time is obviously 0. 2 - In the second case, the specifications
       * mentions that "For children of a sequence, the only legal value for begin is a (single) non-negative offset
       * value. The default begin value is 0." 3 - In the third case, the specification mentions that
    ...
    if (!((len == 1) && (endTimeStringList[0].length() == 0))) { // Ensure the end field is set.
      ...
      for (int i = 0; i < len; i++) {
        try {
          ...
          // Ignore badly formatted times
          logger.error("Malformed time value.", e);
        }
      }
    }
  ...
  public short getFill() {
    ...
    if (fill.equalsIgnoreCase(FILL_FREEZE_ATTRIBUTE)) {
      ...
      /*
       * fill = default The fill behavior for the element is determined by the value of the fillDefault attribute. This
    ...
    /*
     * fill = auto The fill behavior for this element depends on whether the element specifies any of the attributes
     * that define the simple or active duration: - If none of the attributes dur, end, repeatCount or repeatDur are
     * specified on the element, then the element will have a fill behavior identical to that if it were specified as
     * "freeze". - Otherwise, the element will have a fill behavior identical to that if it were specified as "remove".
     */
    if (((mSmilElement.getAttribute("dur").length() == 0) && (mSmilElement.getAttribute("end").length() == 0)
            && (mSmilElement.getAttribute("repeatCount").length() == 0) && (mSmilElement.getAttribute("repeatDur")
            .length() == 0)) || beginAndEndAreZero()) {
      return FILL_FREEZE;
    } else {
      return FILL_REMOVE;
    }
  ...
  public short getFillDefault() {
    ...
    if (fillDefault.equalsIgnoreCase(FILL_REMOVE_ATTRIBUTE)) {
      ...
      /*
       * fillDefault = inherit Specifies that the value of this attribute (and of the fill behavior) are inherited from
       * the fillDefault value of the parent element. This is the default value.
       */
      ElementTime parent = getParentElementTime();
      if (parent == null) {
        /*
         * fillDefault = auto If there is no parent element, the value is "auto".
         */
        return FILL_AUTO;
      } else {
