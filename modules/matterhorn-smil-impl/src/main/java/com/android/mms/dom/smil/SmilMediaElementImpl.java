...
public class SmilMediaElementImpl extends SmilElementImpl implements SMILMediaElement {
  ...
  /** The logger */
  private static final Logger logger = LoggerFactory.getLogger(SmilMediaElementImpl.class);
  ...
  ElementTime mElementTime = new ElementTimeImpl(this) {
    private Event createEvent(String eventType) {
      ...
      if (LOCAL_LOGV) {
        logger.info(TAG, "Dispatching 'begin' event to " + SmilMediaElementImpl.this.getTagName() + " "
                + SmilMediaElementImpl.this.getSrc() + " at " + System.currentTimeMillis());
      }
      return event;
    }
    ...
    public float getDur() {
      ...
      if (dur == 0) {
        ...
        if (tag.equals("video") || tag.equals("audio")) {
          ...
          logger.warn(TAG, "Unknown media type");
        }
      }
      return dur;
    }
