...
/**
 * The SmilPlayer is responsible for playing, stopping, pausing and resuming a SMIL tree. <li>It creates a whole
 * timeline before playing.</li> <li>The player runs in a different thread which intends not to block the main thread.</li>
 */
public class SmilPlayer implements Runnable {

  /** The logging facility */
  private static final Logger logger = LoggerFactory.getLogger(SmilPlayer.class);
  ...
  private static enum SmilPlayerState {
    INITIALIZED, PLAYING, PLAYED, PAUSED, STOPPED,
  }

  private static enum SmilPlayerAction {
    NO_ACTIVE_ACTION, RELOAD, STOP, PAUSE, START, NEXT, PREV
  }
  ...
  private static ArrayList<TimelineEntry> getParTimeline(ElementParallelTimeContainer par, double offset,
          double maxOffset) {
  ...
  private static ArrayList<TimelineEntry> getSeqTimeline(ElementSequentialTimeContainer seq, double offset,
          double maxOffset) {
    ArrayList<TimelineEntry> timeline = new ArrayList<TimelineEntry>();
    ...
    if (element instanceof ElementParallelTimeContainer) {
      ...
      for (int i = 0; i < beginList.getLength(); ++i) {
        ...
        if (begin.getResolved()) {
          double beginOffset = begin.getResolvedOffset() + offset;
          if (beginOffset <= maxOffset) {
            TimelineEntry entry = new TimelineEntry(beginOffset, element, TimelineEntry.ACTION_BEGIN);
            timeline.add(entry);
          }
        }
      }
      ...
      for (int i = 0; i < endList.getLength(); ++i) {
        ...
        if (end.getResolved()) {
          double endOffset = end.getResolvedOffset() + offset;
          if (endOffset <= maxOffset) {
            TimelineEntry entry = new TimelineEntry(endOffset, element, TimelineEntry.ACTION_END);
            timeline.add(entry);
          }
        }
      }
  ...
  public synchronized void play() {
    if (!isPlayingState()) {
      ...
      logger.warn("Error State: Playback is playing!");
    }
  }
  ...
  public synchronized void pause() {
    if (isPlayingState()) {
      ...
      logger.warn("Error State: Playback is not playing!");
    }
  }
  ...
  public synchronized void start() {
    if (isPausedState()) {
      ...
      logger.warn("Error State: Playback can not be started!");
    }
  }
  ...
  private synchronized boolean isBeginOfSlide(TimelineEntry entry) {
    return (TimelineEntry.ACTION_BEGIN == entry.getAction()) && (entry.getElement() instanceof SmilParElementImpl);
  }
  ...
  private synchronized void seekActiveMedia() {
    for (int i = mActiveElements.size() - 1; i >= 0; i--) {
      ...
      if ((offset >= 0) && (offset <= mCurrentTime)) {
        if (LOCAL_LOGV) {
          logger.info("[SEEK]  " + " at " + mCurrentTime + " " + element);
        }
  ...
  private synchronized void waitForEntry(long interval) throws InterruptedException {
    if (LOCAL_LOGV) {
      logger.info("Waiting for " + interval + "ms.");
    }
  ...
  private synchronized void endActiveElements() {
    for (int i = mActiveElements.size() - 1; i >= 0; i--) {
      ElementTime element = mActiveElements.get(i);
      if (LOCAL_LOGV) {
        logger.info("[STOP]  " + " at " + mCurrentTime + " " + element);
      }
      element.endElement();
    }
  }
  ...
  private synchronized void pauseActiveElements() {
    for (int i = mActiveElements.size() - 1; i >= 0; i--) {
      ElementTime element = mActiveElements.get(i);
      if (LOCAL_LOGV) {
        logger.info("[PAUSE]  " + " at " + mCurrentTime + " " + element);
      }
  ...
  private synchronized void resumeActiveElements() {
    ...
    for (int i = 0; i < size; i++) {
      ElementTime element = mActiveElements.get(i);
      if (LOCAL_LOGV) {
        logger.info("[RESUME]  " + " at " + mCurrentTime + " " + element);
      }
  ...
  private synchronized void actionEntry(TimelineEntry entry) {
    switch (entry.getAction()) {
      case TimelineEntry.ACTION_BEGIN:
        if (LOCAL_LOGV) {
          logger.info("[START] " + " at " + mCurrentTime + " " + entry.getElement());
        }
      ...
      case TimelineEntry.ACTION_END:
        if (LOCAL_LOGV) {
          logger.info("[STOP]  " + " at " + mCurrentTime + " " + entry.getElement());
        }
  ...
  private void stopCurrentSlide() {
    ...
    for (int i = mCurrentElement; i < totalEntries; i++) {
      ...
      if (entry.getElement() instanceof SmilParElementImpl && action == TimelineEntry.ACTION_END) {
        actionEntry(entry);
        mCurrentElement = i;
        break;
  ...
  public void run() {
    ...
    for (mCurrentElement = 0; mCurrentElement < size; mCurrentElement++) {
      ...
      while (offset > mCurrentTime) {
        ...
        while (isPauseAction() || isStopAction() || isReloadAction() || isNextAction() || isPrevAction()) {
          if (isPauseAction()) {
            actionPause();
            waitForWakeUp();
          }
