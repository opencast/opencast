...
var ocScheduler = (function() {
  ...
  /**
   *  LoadCAInfo calls cainfo.json to load user friendly CA names
   *  It is synchronous to prevent multiple get requests
   */
  function LoadCAInfo()
  {
      var result,
  ...
  sched.handleAgentList = function(data) {
      ...
      var online = LoadCAInfo();
      $.each(agents, function(i, agent) {
	      if (online[agent.name]) {
		  $(sched.agentList).append($('<option></option>').val(agent.name).html(online[agent.name]));
	      }else{
  ...
  sched.eventSubmitComplete = function(xhr, status) {
      if(status === "success") {
          ...
          window.location.href = "/ltitools/manage/index.html?sid=" + ocUtils.getURLParam('sid') + "&upload=" + ocUtils.getURLParam('upload');
      }
  }
