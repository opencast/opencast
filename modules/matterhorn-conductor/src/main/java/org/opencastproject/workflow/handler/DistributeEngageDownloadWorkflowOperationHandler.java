...
public class DistributeEngageDownloadWorkflowOperationHandler extends AbstractWorkflowOperationHandler {
   ...
   *          the distribution service
   */
  protected void setDistributionService(DownloadDistributionService distributionService) {
  ...
  static {
    ...
    CONFIG_OPTIONS.put("check-availability",
                       "( true | false ) defaults to true. Check if the distributed artifact is available at its URL");
  }
  ...
  public WorkflowOperationResult start(final WorkflowInstance workflowInstance, JobContext context)
    ...
    final WorkflowOperationInstance woi = workflowInstance.getCurrentOperation();
    String sourceTags = StringUtils.trimToNull(woi.getConfiguration("source-tags"));
    String targetTags = StringUtils.trimToNull(woi.getConfiguration("target-tags"));
    String sourceFlavors = StringUtils.trimToNull(woi.getConfiguration("source-flavors"));
    String sourcePriorityFlavors = StringUtils.trimToNull(woi.getConfiguration("source-priority-flavors"));
    boolean checkAvailability = option(woi.getConfiguration("check-availability")).bind(trimToNone).map(toBool).getOrElse(true);
    ...
    try {
      ...
      try {
        for (String elementId : elementIds) {
          Job job = distributionService.distribute(mediaPackage, elementId, checkAvailability);
          if (job == null)
            continue;
