 ...
 */
public class EpisodesPermissionsUpdatedEventHandler implements EventHandler {

  /** The logger */
  protected static final Logger logger = LoggerFactory.getLogger(EpisodesPermissionsUpdatedEventHandler.class);
    ...
    executor.execute(new Runnable() {
      ...
      public void run() {
        ...
        try {
          ...
          EpisodeQuery q = new EpisodeQuery().withSeriesId(seriesId);
          SearchResult result = episodeService.getForAdministrativeRead(q);

          for (SearchResultItem item : result.getItems()) {
            String org = item.getOrganization();
            Organization organization = organizationDirectoryService.getOrganization(org);
            if (organization == null) {
              logger.warn("Skipping update of episode {} since organization {} is unkown", item.getId(), org);
              continue;
            }
