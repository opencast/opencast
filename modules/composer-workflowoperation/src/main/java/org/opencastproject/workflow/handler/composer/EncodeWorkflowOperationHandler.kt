 ...
 */
class EncodeWorkflowOperationHandler : AbstractWorkflowOperationHandler() {
    ...
    override fun start(workflowInstance: WorkflowInstance, context: JobContext): WorkflowOperationResult {
        logger.debug("Running parallel encoding workflow operation on workflow {}", workflowInstance.id)
    ...
    private fun encode(src: MediaPackage, operation: WorkflowOperationInstance): WorkflowOperationResult {
        ...
        for (track in elements) {
            ...
            for (profile in profiles) {
                ...
                encodingJobs[composerService!!.parallelEncode(track, profile.identifier)] = JobInformation(track, profile)
            }
        }
        ...
        for ((job, value) in encodingJobs) {
            ...
            if (job.payload.length > 0) {
                ...
                for (encodedTrack in composedTracks) {
                ...
                if (targetFlavor != null) {
                    ...
                    for (encodedTrack in composedTracks) {
                        encodedTrack.flavor = MediaPackageElementFlavor(flavorType!!, flavorSubtype!!)
                        logger.debug("Composed track {} has flavor '{}'", encodedTrack.toString(), encodedTrack.flavor)
                    }
                }
                ...
                for (encodedTrack in composedTracks) {
                    ...
                    val fileName = getFileNameFromElements(track, encodedTrack)
                    encodedTrack.setURI(workspace!!.moveTo(encodedTrack.getURI(), mediaPackage.identifier.toString(),
                            encodedTrack.identifier, fileName))
                }
            }
        }
    ...
    companion object {
        ...
        private val logger = LoggerFactory.getLogger(EncodeWorkflowOperationHandler::class.java)
    }
