...
class DemuxWorkflowOperationHandlerTest {
    private var operationHandler: DemuxWorkflowOperationHandler? = null
    ...
    fun setUp() {
        ...
        // test resources
        val uriMP = InspectWorkflowOperationHandler::class.java.getResource("/demux_mediapackage.xml").toURI()
        val uriMPEncode = InspectWorkflowOperationHandler::class.java.getResource("/demux_encode_mediapackage.xml").toURI()
        ...
        // set up service
        operationHandler = DemuxWorkflowOperationHandler()
    ...
    fun testComposeEncodedTrack() {
        ...
        EasyMock.expect(composerService!!.demux(EasyMock.anyObject<Any>() as Track, EasyMock.anyObject<Any>() as String))
                .andReturn(job)
        ...
        configurations["encoding-profile"] = PROFILE_ID
        ...
        var trackEncoded = mpNew.getTrack(ENCODED_TRACK_ID)
        Assert.assertEquals("presenter/work", trackEncoded.flavor.toString())
        ...
        Assert.assertEquals("presentation/work", trackEncoded.flavor.toString())
        Assert.assertArrayEquals(targetTags.split("\\W".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray(), trackEncoded.tags)
        Assert.assertEquals(SOURCE_TRACK_ID, trackEncoded.reference.identifier)
    }
    ...
    companion object {
        ...
        private val PROFILE_ID = "demux.work"
        private val SOURCE_TRACK_ID = "demux-workflow-operation-test-source-track-id"
        private val ENCODED_TRACK_ID = "demux-workflow-operation-test-encode-track-id"
        private val ENCODED_TRACK_ID2 = "demux-workflow-operation-test-encode-track-id2"
    }
